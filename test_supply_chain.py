...
def main():
    ...
    org_id = nuchain.create_organization("SEEDS", "Lorem Ipsum", BOB.ss58_address, account=ALICE)
    ...
    nuchain.update_status(tracking_id, "dipanen", int(time.time() * 1000), account=BOB)
    ...
    resp = nuchain.conn.query('ProductTracking', 'EventsOfTracking', params=[tracking_id])
    events = [nuchain.conn.query('ProductTracking', 'AllEvents', params=[str(idx)]) for idx in resp.elements]
    print("Track ID:", tracking_id)
    for event in events:
        event = event.value
        # print(event)
        event_type = list(event['event_type'])[0][8:]
        print("     ├─■ %s [%s] - %s @ location: %s" % (format_time(event['timestamp']), event_type, event['status'], event['location']))
        print("     │")
